...
EXPERIMENTS = [
    ...
    # ),
    (
        "medium_cardinality_absent",
        "medium_cardinality",
        "absent",
        "CREATE TABLE events_medium_cardinality AS SELECT id, CASE WHEN cast(rand()*4000000 as int) = 2000000 THEN 2000001 ELSE cast(rand()*4000000 as int) END AS rand_id, substr(md5(...),1,20) AS rand_str FROM range(200000000)",
        "SELECT * FROM events_medium_cardinality WHERE rand_id = 2000000",
        "200,000,000 records (rand_id=2000000 excluded)",
    ),
    # (
