...
/**
 * BloomFilterAgg generates a DataSketches BloomFilter for a column.
 * The values fed to the filter are converted to bytes using Iceberg's single value serialization,
 * consistent with how ThetaSketchAgg encodes values.
 * The result returned is an array of bytes of a serialized DataSketches BloomFilter,
 * which should be deserialized with BloomFilter.heapify(Memory) before using.
 *
 * The filter is sized using expectedDistinct and targetFpp so that partial aggregates from
 * different tasks are all compatible for union (they share the same bit-array dimensions).
 *
